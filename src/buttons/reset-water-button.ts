...
export class ResetGreenhouseButton implements Button {
    public ids = [GreenhouseUtils.RESET_BUTTON_ID];
    ...
    public async execute(intr: ButtonInteraction, _data: EventData): Promise<void> {
        await this.greenhouseService.reset();
        await InteractionUtils.editReply(intr, GreenhouseUtils.buildMessage(undefined));
    }
}
